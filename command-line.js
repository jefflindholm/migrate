...
function help(options) {
  ...
  for(option of options) {
    let typeName;
    switch(option.type) {
      case Boolean: typeName = 'toggle on'; break;
      case String: typeName = '<string>'; break;
      case Array: typeName = '<string>, allows multiple values by specifying multiple times'; break;
    ...
    const valid = option.options ? '<' + option.options.reduce((p, c ,i) => p + (i > 0 ?', ':'') + c, 'one of: [') + ']>': '';
    console.log(`${name}${sep}${alias} ${typeName}${valid}`)
  }
