...
async function command(sql, args = []) {
  if (config.verbose) console.log(`executing ${sql} args ${args}`);
  return await db.command(sql, args);
}

async function select(sql, args = []) {
  if (config.verbose) console.log(`executing select ${sql} args ${args}`);
  return await db.select(sql, args);
}
...
async function rollback(migrationId, options = {}, migrationDir = null) {
  await configure(options);
  ...
  if (migrationDir) {
    ...
    const details = await select(cmd, [migrationId]);
